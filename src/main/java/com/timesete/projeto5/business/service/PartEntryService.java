...
public class PartEntryService {
    ...
    public PartEntryResponse create(PartEntryRequest request, Integer amount) throws Exception {
        ...
        if (location == null) {
            sublocation = sublocationRepository.findSublocationById(request.getLocationId())
                    .orElseThrow(() -> new Exception("Location or Sublocation not found."));
        }

        for (int i = 0; i < amount; i++) {
            IUPartResponse iupartResponse = iuPartService.createIUPart(request.getPartId(),
        ...
        return PartEntryResponse.builder().build();
    }
